document.getElementById("categoryButton").addEventListener("click",()  => {
    if (screen.width <= 420){
        document.querySelector(".categoryBar").style.right = "0"
        document.querySelector(".categoryBar").style.display = "block"
    }else {
        document.querySelector(".categoryBar").style.right = "0"
    }
})

document.querySelector(".closeCategoryBar").addEventListener("click", () => {

    if (screen.width <= 420){
        document.querySelector(".categoryBar").style.display = "none"
    }else {
        document.querySelector(".categoryBar").style.right = "-1500px"
    }
})
...
fetch("https://crossorigin.me/http://prendjovp-001-site1.atempurl.com/lookup/categories/landing/page/list", {
    ...
    .then(data2 => {
        ...
        generateCategoryBar(fetchedData)
    })
    .catch((error) => {
